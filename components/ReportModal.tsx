...
export default function ReportModal({ deviceId, roomId, onClose }: ReportModalProps) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const isCsmPdf = deviceId.toLowerCase().startsWith('csm')
            const isMlh = deviceId.toLowerCase().startsWith('mlh') || isCsmPdf
            const roomName = isAlarm ? 'Alarm' : isCsmPdf ? '' : `${isMlh ? 'Machine' : 'Room'} ${roomId}`
    ...
    const generatePDF = useCallback(async () => {
        ...
        try {
            ...
            const roomLabel = deviceId.toLowerCase().startsWith('csm') ? '' : `${deviceId.toLowerCase().startsWith('mlh') ? 'Machine' : 'Room'} ${roomId}`
            ...
            const lines = [
                ...
                ...(roomLabel ? [{ text: `${deviceId.toLowerCase().startsWith('mlh') ? 'Machine' : 'Room'}:         ${roomLabel}`, bold: false }] : []),
            ...
            const fileName = `Mech Air_Report_${deviceId.toLowerCase().startsWith('mlh') ? 'Machine' : 'Room'}${roomId}_${selectedRange}_${Date.now()}.pdf`
            doc.save(fileName)
        } catch (e) {
...
const isMlh = deviceId.toLowerCase().startsWith('mlh') || deviceId.toLowerCase().startsWith('csm')
            ...
            const infoRows = [
                ...
                { 'Field': deviceId.toLowerCase().startsWith('mlh') ? 'Machine' : 'Room', 'Value': `${deviceId.toLowerCase().startsWith('mlh') ? 'Machine' : 'Room'} ${roomId}` },
...
<p className="text-xs text-gray-400">{roomId === 's7' || roomId === 'alarm' ? 'Alarm Sensor' : deviceId.toLowerCase().startsWith('csm') ? '' : `${deviceId.toLowerCase().startsWith('mlh') ? 'Machine' : 'Room'} ${roomId} · `}PDF with charts & statistics</p>
                        </div>
                    </div>
