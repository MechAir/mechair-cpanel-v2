    ...
    <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
      ...
      <div className="flex items-center gap-2">
        <input type="number" value={value} step={step ?? 1} min={min} max={max}
          onChange={e => { const raw = e.target.value; if (raw === '' || raw === '-') return; const v = step && step < 1 ? parseFloat(raw) : parseInt(raw, 10); if (isNaN(v)) return; if (min !== undefined && v < min) return; if (max !== undefined && v > max) return; onChange(String(v)) }}
