          ...
          <tbody>
            {steps.map((step, i) => (
              <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                ...
                <td className="py-2 px-4 text-center">{i === steps.length - 1 ? <span className="font-bold text-[#2B8DB8]">Final</span> : <input type="number" min={1} max={29} value={step.days} onChange={e => { const v = Number(e.target.value); if (v < 0 || v > 29) return; updateStep(i, 'days', v) }} className="w-16 text-center rounded-lg border-2 border-[#2B8DB8] px-2 py-1 text-sm font-semibold bg-gray-50 focus:outline-none" />}</td>
