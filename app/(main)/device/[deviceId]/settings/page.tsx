    ...
    <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
      ...
      <div className="flex items-center gap-2">
        <input type="number" inputMode="numeric" value={value} step={step ?? 1} min={min} max={max}
          onChange={e => { if (e.target.value === '' || e.target.value === '-') { onChange('0'); return; } const v = step && step < 1 ? parseFloat(e.target.value) : parseInt(e.target.value, 10); if (isNaN(v)) return; if (min !== undefined && v < min) return; if (max !== undefined && v > max) return; onChange(String(v)) }}
    ...
    <div className="px-3 sm:px-8 py-4 sm:py-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6 items-start">
        <div className="space-y-5">
          ...
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest pt-3">Comp1 Cooling (Temperature)</p>
    ...
    <div className="px-3 sm:px-8 py-4 sm:py-6">
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
          ...
          <div className="flex items-center gap-2">
              <input type="number" inputMode="decimal" step={0.1} min={-100} max={100} defaultValue={cur.humidityOffset} key={`${activeRoom}-humid-${cur.humidityOffset}`} readOnly={readOnly} disabled={readOnly}
              onBlur={e => { const v = parseFloat(e.target.value); update({ humidityOffset: isNaN(v) ? 0 : Math.max(-100, Math.min(100, v)) }) }}
              className={`w-28 text-center text-lg font-semibold text-gray-800 border-2 border-[#9C27B0] rounded-xl py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-[#9C27B0]/40 bg-gray-50 ${readOnly ? 'opacity-70 cursor-not-allowed' : ''}`} />
    ...
    <div className="px-3 sm:px-8 py-4 sm:py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6">
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3"><label className="text-gray-700 text-sm sm:text-base font-medium sm:w-48 sm:shrink-0">Temp High Limit:</label><div className="flex items-center gap-2"><input type="number" step={0.5} min={-40} max={60} defaultValue={settings.tempHigh} key={settings.tempHigh} readOnly={readOnly} disabled={readOnly} onBlur={e => { const v = parseFloat(e.target.value); update({ tempHigh: isNaN(v) ? 0 : Math.max(-40, Math.min(60, v)) }) }} className={`w-28 text-center text-lg font-semibold text-gray-800 border-2 border-[#7C3AED] rounded-xl py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/40 bg-gray-50 ${readOnly ? 'opacity-70 cursor-not-allowed' : ''}`} /><span className="bg-[#7C3AED] text-white text-sm font-bold px-4 py-2.5 rounded-xl min-w-[52px] text-center">°C</span></div></div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3"><label className="text-gray-700 text-sm sm:text-base font-medium sm:w-48 sm:shrink-0">Temp Low Limit:</label><div className="flex items-center gap-2"><input type="number" step={0.5} min={-40} max={60} defaultValue={settings.tempLow} key={settings.tempLow} readOnly={readOnly} disabled={readOnly} onBlur={e => { const v = parseFloat(e.target.value); update({ tempLow: isNaN(v) ? 0 : Math.max(-40, Math.min(60, v)) }) }} className={`w-28 text-center text-lg font-semibold text-gray-800 border-2 border-[#7C3AED] rounded-xl py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/40 bg-gray-50 ${readOnly ? 'opacity-70 cursor-not-allowed' : ''}`} /><span className="bg-[#7C3AED] text-white text-sm font-bold px-4 py-2.5 rounded-xl min-w-[52px] text-center">°C</span></div></div>
  ...
  return (
    <div className="px-3 sm:px-8 py-4 sm:py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6">
        ...
        <div className="space-y-5">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Temperature Limits</p>
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3"><label className="text-gray-700 text-sm sm:text-base font-medium sm:w-48 sm:shrink-0">Temp High Limit:</label><div className="flex items-center gap-2"><input type="number" step={0.5} min={-50} max={100} defaultValue={cur.tempHigh ?? 30} key={cur.tempHigh} readOnly={readOnly} disabled={readOnly} onBlur={e => { const v = parseFloat(e.target.value); update({ tempHigh: isNaN(v) ? 0 : Math.max(-50, Math.min(100, v)) }) }} className={`w-28 text-center text-lg font-semibold text-gray-800 border-2 border-[#2B8DB8] rounded-xl py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-[#2B8DB8]/40 bg-gray-50 ${readOnly ? 'opacity-70 cursor-not-allowed' : ''}`} /><span className="bg-[#2B8DB8] text-white text-sm font-bold px-4 py-2.5 rounded-xl min-w-[52px] text-center">°C</span></div></div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3"><label className="text-gray-700 text-sm sm:text-base font-medium sm:w-48 sm:shrink-0">Temp Low Limit:</label><div className="flex items-center gap-2"><input type="number" step={0.5} min={-50} max={100} defaultValue={cur.tempLow ?? -5} key={cur.tempLow} readOnly={readOnly} disabled={readOnly} onBlur={e => { const v = parseFloat(e.target.value); update({ tempLow: isNaN(v) ? 0 : Math.max(-50, Math.min(100, v)) }) }} className={`w-28 text-center text-lg font-semibold text-gray-800 border-2 border-[#2B8DB8] rounded-xl py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-[#2B8DB8]/40 bg-gray-50 ${readOnly ? 'opacity-70 cursor-not-allowed' : ''}`} /><span className="bg-[#2B8DB8] text-white text-sm font-bold px-4 py-2.5 rounded-xl min-w-[52px] text-center">°C</span></div></div>
